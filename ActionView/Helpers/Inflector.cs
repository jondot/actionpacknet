...
//
// Some modifications to source.
// Copyright 2008 Nahum Dotan.
...
namespace ActionPack.Helpers
{
    ...
    public class Inflector
    {
        ...
        private static readonly Regex camelize = new Regex(@"(^|[\s_\.])(.)", RegexOptions.Compiled);
       
        public static string Camelize(string sentence, bool isLowerCamel)
        {
            if (string.IsNullOrEmpty(sentence)) return "";
            string res = camelize.Replace(sentence, x => x.Groups[2].Value.ToUpper());
            return isLowerCamel ? char.ToLower(res[0]) + res.Substring(1) : res;
            
        }

        private static readonly Regex dasherize = new Regex(@"[\s_\.](.)", RegexOptions.Compiled);
        public static string Dasherize(string sentence)
        {
            return dasherize.Replace(sentence.Trim(), "-$1");
        }

        
        public static string Decamelize(string word, string separator)
        {
            string res = Regex.Replace(word, "([A-Z]+)([A-Z][a-z])", "$1" + separator + "$2");
            return Regex.Replace(res, @"([a-z\d])([A-Z])", "$1" + separator + "$2");
        }
        ...
        {
            return Thread.CurrentThread.CurrentUICulture.TextInfo.ToTitleCase(sentence.ToLower(System.Globalization.CultureInfo.InvariantCulture));
        }
